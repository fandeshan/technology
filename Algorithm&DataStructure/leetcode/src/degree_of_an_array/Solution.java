...
public class Solution {
    public int findShortestSubArray(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            List<Integer> indexLists = indexMap.getOrDefault(nums[i],new ArrayList<>());
            indexLists.add(i);
            indexMap.put(nums[i] ,indexLists);
        }
        int maxCount = 0;
        int minLen = 0;
        for (Map.Entry<Integer,List<Integer>> entry:indexMap.entrySet()) {
            int currLen = entry.getValue().get(entry.getValue().size() - 1) - entry.getValue().get(0) + 1;
