...
public class MagicDictionary {
    ...
    public void buildDict(String[] dictionary) {
        ...
        for (int i = 0; i < dictionary.length; i++) {
            List<String> currDict = dictMap.getOrDefault(dictionary[i].length(),new ArrayList<>());

            currDict.add(dictionary[i]);
            dictMap.put(dictionary[i].length(),currDict);
        }
    }
    ...
    public boolean search(String searchWord) {
        ...
        if (!dictMap.containsKey(searchWord.length())){
            return false;
        }
        List<String> currDict = dictMap.get(searchWord.length());
        for (int i = 0; i < currDict.size(); i++) {
            int diff = 0;
            for (int j = 0; j < currDict.get(i).length(); j++) {
                if (searchWord.charAt(j)!=currDict.get(i).charAt(j)){
                    diff ++;
                    if (diff > 1){
                        break;
                    }
