...
public class Solution {
    public int findMaximizedCapital(int k, int w, int[] profits, int[] capital) {
        ...
        quickSort(capital,profits,0,n-1);

        PriorityQueue<Integer> queue = new PriorityQueue<Integer>((a,b) -> b-a);
        int j = 0;
        for (int i = 0; i < k; i++) {
            while( j<n && capital[j] <= w){
                queue.offer(profits[j]);
                j ++;
            }
            if (!queue.isEmpty()){
                w += queue.poll();
    ...
    private void quickSort(int[] num,int[] num1,int start,int end){
        if (end <= start) {
            return ;
        }
        int left = start;
        ...
        quickSort(num,num1,start,right);
        quickSort(num,num1,left,end);
    }
