...
public class Solution {
    class Graph{
        int index;
        List<Graph> pools = new ArrayList<>();
        int quietIndex;
    }
    public int[] loudAndRich(int[][] richer, int[] quiet) {
        int[] answers = new int[quiet.length];
        int[][] quietSort = new int[quiet.length][2];
        for (int i = 0; i < quiet.length; i++) {
            quietSort[i][0] = quiet[i];
            quietSort[i][1] = i;
        }
        quickSort(quietSort,0,quietSort.length-1);
        ...
        for (int i = 0; i < richer.length; i++) {
            graphs.get(richer[i][0]).pools.add(graphs.get(richer[i][1]));
        }
        // mark if reached
        int[] reached = new int[quiet.length];
        for (int i = 0; i < quietSort.length; i++) {
            dfs(graphs.get(quietSort[i][1]),quietSort[i][1],reached);
        }
        //dfs(graphs,-1,quiet,reached);
        for (int i = 0; i < graphs.size(); i++) {
            answers[i] = graphs.get(i).quietIndex;
        }
...
//        System.out.println();
    ...
    private void dfs(Graph graph,int minQuietIndex,int[] reached){
        if (reached[graph.index] == 1){
            return ;
        }
        reached[graph.index] = 1;
        graph.quietIndex = minQuietIndex;
        for (int i = 0; i < graph.pools.size(); i++) {
            dfs(graph.pools.get(i),minQuietIndex,reached);
        }
    }
