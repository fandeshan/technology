...
public class Solution {
    public int numWaterBottles(int numBottles, int numExchange) {
        return getNumWaterBottles(numBottles,0,numExchange);
    }
    private int getNumWaterBottles(int numBottles,int lessNumBottles, int numExchange){
        int totalBottles = numBottles + lessNumBottles;
        if (totalBottles < numExchange){
            return numBottles;
        }
        return numBottles + getNumWaterBottles(totalBottles/numExchange,totalBottles%numExchange,numExchange);
    }
