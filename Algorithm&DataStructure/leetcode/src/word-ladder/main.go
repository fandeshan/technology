...
func main() {
	...
	//beginWord := "hit"
	//endWord := "cog"
	//wordList := []string{"hot","hit","cog","dot","dog"}
	...
	//wordList := []string{"hot","dot","lot","cog"}
	//beginWord := "qa"
	//endWord := "sq"
...
func ladderLength(beginWord string, endWord string, wordList []string) int {
	endIndex := findEndIndex(wordList,endWord)
	if endIndex == -1 {
		return 0
	}
	t1 := time.Now()
	transFromMap := make([][]bool,len(wordList)+1)
	for i := 0 ; i < len(transFromMap) ; i++  {
		transFromMap[i] =make([]bool,len(wordList))
		for j := 0;j < len(transFromMap[i]);j ++ {
			if i == 0 {
				transFromMap[i][j] = canTransfrom(beginWord,wordList[j])
			}else{
				transFromMap[i][j] = canTransfrom(wordList[i-1],wordList[j])
			}
		}
	}
	...
	min := getMinEnd(transFromMap,endIndex,minMap)
	fmt.Println(time.Since(t2))
	if min == len(transFromMap) {
		return 0
	}else {
		return min +1
	}
...
func getMinEnd(transFromMap [][]bool,startIndex int,minMap map[int]int) int{
	if startIndex == -1 {
		return 0
	}
	if minStep,ok := minMap[startIndex];ok {
		return minStep
	}
	minStep := len(transFromMap)
	for i:=0;i<len(transFromMap);i++  {
		if transFromMap[i][startIndex] {
			//hasFinded := false
			//for j:=0;j<len(findedList) ;j++  {
			//	if findedList[j] == i-1 {
			//		hasFinded = true
			//		break
			//	}
			...
			//	continue
			//}
			//findedList = append(findedList,i-1)
			//fmt.Println(findedList)
			transFromMap[i][startIndex] = false
			if i>0 {
				transFromMap[startIndex+1][i-1] = false
			}

			nextMinStep := getMinEnd(transFromMap,i-1,minMap)
			transFromMap[i][startIndex] = true
			if i>0 {
				transFromMap[startIndex+1][i-1] = true
			}
			//findedList = findedList[:len(findedList)-1]
			if minStep > nextMinStep+1 {
				minStep = nextMinStep+1
				minMap[startIndex] = minStep
			}
		}
	}
	return minStep
...
func ladderLength2(beginWord string, endWord string, wordList []string) int {
	...
	minToStartMap :=make(map[string]int)
	//minToStartMap1 :=make(map[string]string)
	first := true
	//stepByStep := 2
	...
	for len(wordList)>0  {
		meetNext := false
		newMinToStartMap := make(map[string]int)
		for i:=0;i<len(wordList);i++  {
			if first {
				if canTransfrom(beginWord,wordList[i]) {
					meetNext = true
					minToStartMap[wordList[i]] = 2
					//minToStartMap1[wordList[i]] = beginWord+","+wordList[i]
					if wordList[i] == endWord {
						return minToStartMap[wordList[i]]
					}
				...
				//currKeyMin := ""
				for k,v := range minToStartMap {
					//if v >= stepByStep{
					//	continue
					//}
					if canTransfrom(k,wordList[i]) {
						meetNext = true
						if v + 1 < currMin {
							currMin = v + 1
							//currKeyMin = minToStartMap1[k]
						}
					}
				}
				if currMin< math.MaxInt32 {
					newMinToStartMap[wordList[i]] = currMin
					//minToStartMap1[wordList[i]] = currKeyMin+","+wordList[i]
					if wordList[i] == endWord {
						fmt.Println(time.Since(t1))
						//fmt.Println(minToStartMap1[wordList[i]])
						return newMinToStartMap[wordList[i]]
					}
					wordList = append(wordList[:i],wordList[i+1:]...)
					i--
				}
		...
		for k,v := range newMinToStartMap {
			minToStartMap[k] = v
		}
		//stepByStep ++
	}
...
func findEndIndex(wordList []string,endWord string)  int{
	for i := 0 ;i < len(wordList) ;i ++  {
...
//var transfromCacheMap = make(map[string]bool)
func canTransfrom(currWord string,nextWord string) bool {
	//cacheKey := currWord+","+nextWord
	//if result,ok:= transfromCacheMap[cacheKey];ok {
	//	return result
	//}
	cnt := 0
	for i:=0;i<len(currWord) ;i++  {
		if []byte(currWord)[i] != []byte(nextWord)[i] {
			cnt ++
		}
	}
	//transfromCacheMap[cacheKey] = cnt == 1
	return cnt == 1
}
//func getMin(beginWord string, endWord string, wordList []string,minToStartMap map[string]int) bool {
//	fmt.Println(wordList)
//	fmt.Println(minToStartMap)
//	if canTransfrom(beginWord,endWord) {
//		currMin := 1
//		if beginWordMin,ok := minToStartMap[beginWord];ok {
//			currMin = beginWordMin + 1
//		}
//		if currMapMin,ok:=minToStartMap[endWord];ok {
//			if currMin < currMapMin {
//				minToStartMap[endWord] = currMin
//			}
//		}else {
//			minToStartMap[endWord] = currMin
//		}
...
//				currMin = beginWordMin + 1
//			}
//			if currMapMin,ok:=minToStartMap[wordList[i]];ok {
//				if currMin < currMapMin {
//					minToStartMap[wordList[i]] = currMin
//				}
//			}else {
//				minToStartMap[wordList[i]] = currMin
//			}
//			wordList = append(wordList[:i],wordList[i+1:]...)
//			if !getMin(wordList[i],endWord,wordList,minToStartMap){
//				return false
//			}
//			i --
//		}
